...
public:
    ...
    //Dijkstra 알고리즘
    std::pair<std::vector<int>, std::vector<int>> Dijkstra(const Node& start, const Node& end);
};

//Dijkstra의 결과인 prev 벡터를 가져와서 도착 노드부터 지나온 노드들을 거꾸로 돌아오면서 시작 노드까지 노드의 이름을 string 벡터 path에 저장(reverse를 사용해서 순서대로 저장)
...
//출발 노드부터 도착 노드까지 Dijkstra 알고리즘을 통해 거리를 계산
...
std::pair<std::vector<int>, std::vector<int>> Graph::Dijkstra(const Node& start, const Node& end) {
    ...
    std::priority_queue<PQElement, std::vector<PQElement>, std::greater<PQElement>> pq; //Dijkstra를 위한 우선순위 큐 pq 선언.
...
int main() {
    ...
    // Regular search
    auto regularResult = graph.Dijkstra(startNode, endNode);
    ...
    auto noStairsResult = noStairsGraph.Dijkstra(startNode, endNode);
    ...
    auto noElevatorsResult = noElevatorsGraph.Dijkstra(startNode, endNode);
    ...
    auto roadOnlyResult = roadOnlyGraph.Dijkstra(startNode, endNode);
